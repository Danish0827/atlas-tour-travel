...
if (isset($_POST['addPackageDetail'])) {
    ...
    $PackageDetail = PackageDetail($package_name);
        if (mysqli_num_rows($PackageDetail) == 0) {
            $user_query = "INSERT into `hajj_package_detail`(`package_name`, `notes`, `includes`, `requirements`,  `tnc`)  VALUES ('$package_name','$notes','$includes','$requirements','$tnc')";
            $query_run = mysqli_query($con_hajj, $user_query);
            if ($query_run) {
                $_SESSION['status'] = "Package Details Added Successfully";
                header("Location:hajj_package_detail.php");
            } else {
...
if (isset($_POST['updatePackageDetail'])) {
    ...
    if (mysqli_num_rows($PackageDetail) != 0) {
        echo 1;
        $query = "UPDATE `hajj_package_detail` SET `id`='$package_detail_id',`package_name`='$package_name',`notes`='$notes',`includes`='$includes',`requirements`='$requirements',`tnc`='$tnc' Where id='$package_detail_id'";
...
if (isset($_POST['deletePackageDetail'])) {
    ...
    $query = "DELETE from hajj_package_detail where id='$package_detail_id'";
    $query_run = mysqli_query($con_hajj, $query);
    if ($query_run) {
